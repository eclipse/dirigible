...
public class TableMetadata {
   ...
   */
  public TableMetadata(String name, String type, String remarks, Connection connection, String catalogName, String schemaName, boolean deep)
      throws SQLException {
    super();
    this.name = name;
    this.type = type;
    ...
    if (deep) {
      DatabaseMetadataHelper.iterateTableDefinition(connection, catalogName, schemaName, name, new ColumnsIteratorCallback() {
        ...
        public void onColumn(String columnName, String columnType, String columnSize, boolean isNullable, boolean isKey, int scale) {
          columns.add(
              new ColumnMetadata(columnName, columnType, columnSize != null ? Integer.parseInt(columnSize) : 0, isNullable, isKey, scale));
        }
