...
public class CsvimProcessor {
      ...
      if (skipComparing) {
        ...
        if (pkValueForCSVRecord == null) {
          ...
          if (csvRecord.size() != tableColumns.size()) {
            CsvimUtils.logProcessorErrors(String.format(PROBLEM_MESSAGE_DIFFERENT_COLUMNS_SIZE, pkValueForCSVRecord), ERROR_TYPE_PROCESSOR,
                csvFile.getFile(), Csv.ARTEFACT_TYPE, MODULE);
   ...
   * @param csvFile the csv file
   */
  private void insertCsvRecords(Connection connection, TableMetadata tableModel, List<CSVRecord> recordsToProcess, List<String> headerNames,
      CsvFile csvFile) {
    try {
   ...
   * @param csvFile the csv file
   */
  private void updateCsvRecords(Connection connection, TableMetadata tableModel, List<CSVRecord> recordsToProcess, List<String> headerNames,
      String pkName, CsvFile csvFile) {
    try {
      List<CsvRecord> csvRecords = recordsToProcess.stream()
                                                   .map(e -> new CsvRecord(e, tableModel, headerNames,
                                                       csvFile.getDistinguishEmptyFromNull()))
