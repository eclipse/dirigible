...
jobsView.controller('JobsController', ['$scope', '$http', 'messageHub', function ($scope, $http, messageHub) {
	...
	$http.get('/services/v8/ops/jobs').then(function (response) {
		$scope.list = response.data;
	});

	$scope.enable = function (job) {
		$http.post('/services/v8/ops/jobs/enable/' + job.name)
	...
	$scope.disable = function (job) {
		$http.post('/services/v8/ops/jobs/disable/' + job.name)
			.then(function (response) {
	...
	$scope.getEmails = function (job) {
		$scope.job = job;
		$http.get('/services/v8/ops/jobs/emails/' + job.name)
			.then(function (response) {
				$scope.name = job.name;
	...
	$scope.addEmail = function () {
		$http.post('/services/v8/ops/jobs/emailadd/' + $scope.job.name, $scope.job.email)
			.then(function (response) {
	...
	$scope.removeEmail = function (id) {
		$http.delete('/services/v8/ops/jobs/emailremove/' + id)
			.then(function (response) {
				$scope.getEmails($scope.job);
			}, function (response) {
				console.error(response.data);
