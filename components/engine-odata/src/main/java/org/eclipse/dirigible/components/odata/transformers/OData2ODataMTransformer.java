...
public class OData2ODataMTransformer {
        ...
        for (ODataEntity entity : model.getEntities()) {
            ...
            if (entityProperties.isEmpty()) {
                ...
                entityProperties.forEach(prop -> {
                    List<TableColumn> dbColumnName = tableMetadata.getColumns().stream().filter(x -> x.getName().equals(prop.getColumn())).collect(Collectors.toList());
            ...
            Map<String, List<TableConstraintForeignKey>> groupRelationsByToTableName = tableMetadata.getConstraints().getForeignKeys().stream()
            ...
            List<ODataAssociation> assWhereEntityIsFROMRole = model.getAssociations().stream().filter(ass -> ass.getFrom().getEntity().equals(entity.getName())).collect(Collectors.toList());
            List<ODataAssociation> assWhereEntityIsTORole = model.getAssociations().stream().filter(ass -> ass.getTo().getEntity().equals(entity.getName())).collect(Collectors.toList());
     ...
     * @return the string
     */
    private String checkRefSectionConsistency(StringBuilder buff, ODataEntity entity, Map<String,
            List<TableConstraintForeignKey>> groupRelationsByToTableName, List<String> assembleRefFromFK,
                                              ODataEntity toSetEntity, String principleEntity, String fkElement,
    ...
    private void validateAssociationProperty(ODataAssociationEnd assEnd, OData model, ODataAssociation association) {
        ...
        if (!entity.getProperties().isEmpty()) {
            ...
            if (!invalidProps.isEmpty()) {
                throw new OData2TransformerException(String.format("There is inconsistency for entity '%s'. OData entity properties definitions for %s do not match the association properties definition.", assEnd.getEntity(), invalidProps.stream().map(String::valueOf).collect(Collectors.joining(","))));
            }
            if (assEnd.getProperties().size() > entity.getProperties().size()) {
                throw new OData2TransformerException(String.format("There is inconsistency for entity '%s'. The number of defined OData properties do not match the number of the association properties definition", assEnd.getEntity()));
            }
        }
    }
