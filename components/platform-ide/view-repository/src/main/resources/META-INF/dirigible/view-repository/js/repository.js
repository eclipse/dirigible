 ...
 */
const repositoryView = angular.module('repository', ['blimpKit', 'platformView', 'platformShortcuts', 'RepositoryService', 'TransportService']);
repositoryView.constant('StatusBar', new StatusBarHub());
repositoryView.constant('Layout', new LayoutHub());
repositoryView.constant('Dialogs', new DialogHub());
repositoryView.constant('Notifications', new NotificationHub());
repositoryView.constant('Repository', new RepositoryHub());
repositoryView.constant('ContextMenu', new ContextMenuHub());
repositoryView.controller('RepositoryViewController', ($scope, $document, clientOS, StatusBar, Layout, Dialogs, Notifications, ContextMenu, TransportService, Repository, RepositoryService, ButtonStates) => {
    $scope.state = {
        isBusy: true,
        error: false,
        busyText: 'Loading...',
    ...
    const jstreeWidget = angular.element('#dgRepository');
    const jstreeConfig = {
        core: {
            check_callback: true,
            themes: {
        ...
        state: { key: `${brandingInfo.keyPrefix}.view-repository.state` },
        types: {
            'default': {
    ...
    $scope.importRepository = () => {
        Dialogs.showWindow({
            hasHeader: true,
            id: 'import',
            params: { importRepository: true }
        });
    };

    $scope.exportRepository = () => {
        TransportService.exportRepository();
    };
    ...
    $scope.reloadFileTree = (basePath, setConfig = false) => {
        ...
        RepositoryService.load(basePath).then((response) => {
