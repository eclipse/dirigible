...
public class WorkspaceJsonHelper implements InitializingBean {
  ...
  public static WorkspaceDescriptor describeWorkspace(ICollection collection, String removePathPrefix, String addPathPrefix) {
    ...
    for (ICollection childCollection : collections) {
      workspacePojo.getProjects()
                   .add(describeProject(workspacePojo.getName(), childCollection, removePathPrefix, addPathPrefix, projectStatusCache));
    }
  ...
  private static ProjectStatus remapPaths(String project, String git, ProjectStatus status) {
    ProjectStatus result =
        new ProjectStatus(project, git, canonizePaths(git, project, status.getAdded()), canonizePaths(git, project, status.getChanged()),
            canonizePaths(git, project, status.getRemoved()), canonizePaths(git, project, status.getMissing()),
            canonizePaths(git, project, status.getModified()), canonizePaths(git, project, status.getConflicting()),
            canonizePaths(git, project, status.getUntracked()), canonizePaths(git, project, status.getUntrackedFolders()));
    return result;
  }
