...
public class DatabaseFacade implements InitializingBean {
	...
	public static final String getDataSources() {
		return GsonHelper.toJson(DatabaseFacade.get().getDatabaseDefinitionService().getDataSourcesNames());
	}
	...
	public static final DataSource getDefaultDataSource() {
		return DatabaseFacade.get().getDataSourcesManager().getDefaultDataSource();
	}
	...
	private static DataSource getDataSource(String datasourceName) {
		...
		if (datasourceName == null) {
			dataSource = DatabaseFacade.get().getDataSourcesManager().getDefaultDataSource();
		} else {
			dataSource = DatabaseFacade.get().getDataSourcesManager().getDataSource(datasourceName);
		}
		return dataSource;
	}
