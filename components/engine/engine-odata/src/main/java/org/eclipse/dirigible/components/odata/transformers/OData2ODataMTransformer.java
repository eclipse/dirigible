...
public class OData2ODataMTransformer {
      ...
      final List<String> assembleRefFromFK = relationsThatExistInOdataFile.stream()
                                                                          .map(rel -> {
                                                                            ...
                                                                            ODataEntity toSetEntity =
                                                                                ODataMetadataUtil.getEntityByTableName(model, rel.getKey());
      ...
      List<String> assembleRefNav = assWhereEntityIsFROMRole.stream()
                                                            .map(association -> {
                                                              ...
                                                              String toRoleEntity = association.getTo()
                                                                                               .getEntity();
                                                              ODataEntity toSetEntity =
                                                                  ODataMetadataUtil.getEntity(model, toRoleEntity, association.getName());
                                                              String dependentEntity = toSetEntity.getName();
                                                              ...
                                                              return checkRefSectionConsistency(buff, entity, groupRelationsByToTableName,
                                                                  assembleRefFromFK, toSetEntity, dependentEntity, fkElement,
      ...
      assembleRefNav = assWhereEntityIsTORole.stream()
                                             .map(association -> {
                                               ...
                                               String mappingTableJoinColumn = association.getTo()
                                                                                          ...
                                                                                          .getMappingTableJoinColumn();
                                               return checkRefSectionConsistency(buff, entity, groupRelationsByToTableName,
                                                   assembleRefFromFK, fromSetEntity, principleEntity, fkElement, mappingTableName,
  ...
  private String checkRefSectionConsistency(StringBuilder buff, ODataEntity entity,
      Map<String, List<TableConstraintForeignKey>> groupRelationsByToTableName, List<String> assembleRefFromFK, ODataEntity toSetEntity,
      String principleEntity, String fkElement, String mappingTableName, String mappingTableJoinColumn) {
    ...
    if (groupRelationsByToTableName.get(toSetEntity.getTable()) != null) {
      ...
      if (match.isEmpty()) {
        throw new OData2TransformerException(
            String.format("There is inconsistency in odata file from table %s to table %s on joinColumns: %s", entity.getTable(),
                principleEntity, fkElement));
      }
    }
    if (!buff.toString()
  ...
  private String assembleOdataMRefSection(String dependentEntity, String fkElements, String mappingTableName,
    ...
    if (mappingTableName != null && !mappingTableName.isEmpty() && mappingTableJoinColumn != null && !mappingTableJoinColumn.isEmpty()) {
