        ...
        if (bpmn.getLocation()
          ...
          if (processDefinitions.size() > 0) {
            ...
            if (processDefinitions.size() > 0) {
              ...
              bpmn.setProcessDefinitionDescription(processDefinition.getDescription());
              if (logger.isInfoEnabled()) {
                logger.info(format("Updated deployment: [{0}] with key: [{1}] on the Flowable BPMN Engine.", deployment.getId(),
                    deployment.getKey()));
              }
              return;
            }
  ...
  private void removeFromProcessEngine(Bpmn bpmn) {
    ...
    for (Deployment deployment : deployments) {
      ...
      if (bpmn.getLocation()
        ...
        if (logger.isInfoEnabled()) {
          logger.info(
              format("Deleted deployment: [{0}] with key: [{1}] on the Flowable BPMN Engine.", deployment.getId(), deployment.getKey()));
        }
        break;
      }
    }
