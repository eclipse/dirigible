...
JSONPath.prototype._walk = function (loc, expr, val, path, parent, parentPropName, callback, f) {
    if (Array.isArray(val)) {
        ...
        for (var m in val) { // originally m declared as `const`, but this is not allowed in Rhino
...
JSONPath.prototype._slice = function (loc, expr, val, path, parent, parentPropName, callback) {
    ...
    for (let i = start; i < end; i += step) {
        let tmp = this._trace(unshift(i, expr), val, path, parent, parentPropName, callback);// originally declared as `const`, this doesn't work in Rhino
        if (Array.isArray(tmp)) {
