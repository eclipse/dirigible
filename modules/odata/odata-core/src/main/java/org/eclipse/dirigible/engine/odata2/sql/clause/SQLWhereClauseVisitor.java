      ...
      switch (propertyKind) {
        ...
        default:
          throw new OData2Exception(
              String.format("Error during processing member clause between path %s and property %s: Unsupported property kind %s",
  ...
  public Object visitProperty(PropertyExpression propertyExpression, String uriLiteral, EdmTyped edmProperty) {
    try {
      ...
      switch (propertyKind) {
        ...
        default:
          throw new OData2Exception(String.format("Unable to handle PropertyKind %s for EdmType %s and PropertyProvider %s", propertyKind,
              targetType, edmProperty.getClass()
                                     .getName()),
              INTERNAL_SERVER_ERROR);
  ...
  private String writeParam(final CommonExpression expression, final String literal, final String format) {
    ...
    if (expression.getKind() == ExpressionKind.LITERAL) {
      ...
      whereClauseParams.add(
          SQLWhereClause.param(literalValue, (EdmSimpleType) expression.getEdmType(), columnInfo == null ? null : columnInfo.get(0)));
