...
import org.eclipse.dirigible.engine.js.graalium.execution.dependencies.DependencyProvider;
...
public class GraalJSContextCreator {
    ...
    public static Context createContext(
            Engine engine,
            GraalJSContextFolders graalJSContextFolders,
            Consumer<Context.Builder> onBeforeContextCreatedHook, Consumer<Context> onAfterContextCreatedHook
    ) {
        Path workingDirectoryPath = graalJSContextFolders.getWorkingDirectoryPath();
        Path dependenciesCachePath = graalJSContextFolders.getDependenciesCachePath();
        Path coreModulesProxiesCachePath = graalJSContextFolders.getCoreModulesProxiesCachePath();

        DirigibleCoreModuleResolver dirigibleCoreModuleResolver = new DirigibleCoreModuleResolver(coreModulesProxiesCachePath);
        DependencyProvider dependencyProvider = new DependencyProvider(dependenciesCachePath);
        GraalJSFileSystem graalJSFileSystem = new GraalJSFileSystem(workingDirectoryPath, dirigibleCoreModuleResolver, dependencyProvider);
