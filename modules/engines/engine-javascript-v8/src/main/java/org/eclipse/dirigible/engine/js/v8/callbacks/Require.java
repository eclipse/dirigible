...
public class Require {
	...
	public static final String CODE = "var Require = (function(modulePath) {" + "	var _loadedModules = {};" + " var _require = function(path) {"
			...
			+ " moduleInfo = _loadedModules[path];" + " if (moduleInfo) {" + "   return moduleInfo;" + " }" + " code = _j2v8loadSource(path);"
			+ " moduleInfo = {" + "   loaded : false," + "   id : path," + "   exports : {}," + "   require : _requireClosure()" + " };"
			+ " code = head + code + tail;" + " _loadedModules[path] = moduleInfo;" + " var compiledWrapper = null;" + " try {"
			+ "   compiledWrapper = eval(code);" + " } catch (e) {"
