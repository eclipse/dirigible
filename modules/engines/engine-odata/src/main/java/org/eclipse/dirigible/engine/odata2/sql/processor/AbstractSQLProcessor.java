...
public abstract class AbstractSQLProcessor extends ODataSingleProcessor implements SQLProcessor {
    ...
    protected int doCountEntitySet(SQLQuery sqlQuery, final Connection connection) throws ODataException, SQLException {
        ...
        try {
            try {
                ...
                // TODO do we need to assert that resultSet.next() == true and subsequently resultSet.next() == false here?
                resultSet.next();
    ...
    protected PreparedStatement createStatement(SQLQuery query, final Connection connection)
        ...
        if (query.getSelectExpression().getSkip() != SQLExpressionSelect.NOT_SET) {
            ...
            /*
             * Restrict fetch size to prevent OutOfMemoryErrors for huge page
             * sizes. If no fetch size is specified, jConnect will fetch all
             * records up to the requested page (in case of the last page, this
             * would be the entire result set).
             * 
             * Internal Incident:
    ...
    public ODataResponse readEntity(final GetEntityUriInfo uriInfo, final String contentType) throws ODataException {
        ...
        // TODO change logic to read columns from resultset? (key has to be read even if it is now requested via $select)
                        ...
                        if (hasExpand(uriInfo.getExpand())) {//TODO remove the duplication here with the readEntitySet
                            for (List<NavigationPropertySegment> expandContents : uriInfo.getExpand()) {
                                for (NavigationPropertySegment expandContent : expandContents) {
                                    ...
                                    Map<String, Object> expandData = readResultSet(query, expandType, EdmUtils.getProperties(expandType),
                                            resultSet);
                                    if (OData2Utils.isEmpty(expandType, new OData2ResultSetEntity(expandData))) {
                                        //nothing there, we need to find the next one
                                        continue;
                                    }
                                    Map<String, Object> customizedExpandData = onCustomizeExpandedNavigatonProperty(targetEntityType,
                                            expandType, expandData);
    ...
    public ODataResponse readEntitySet(final GetEntitySetUriInfo uriInfo, final String contentType) throws ODataException {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        //for every row, we create one target enttiy instace and compare it with the previous one. If the key property values are different, then we move to the next target instance. 
                        //Otherwise that row has only navigation properties, then in that iteration only the navigation properties are set
                        while (query.next(resultSet)) {//TODO remove the duplication here
                            ...
                            if (!OData2Utils.isSameInstance(targetEntityType, currentResultSetEntity, nextResultSetEntity)) {
                                ...
                                for (List<NavigationPropertySegment> expandContents : uriInfo.getExpand()) {
                                    for (NavigationPropertySegment expandContent : expandContents) {
                                        ...
                                        if (!currentResultSetEntity.getExpandData().containsKey(expandTypeFqn)) {
                                            currentResultSetEntity.getExpandData().put(expandTypeFqn, new ArrayList<Object>());
                                        }
                                        ...
                                        Map<String, Object> customizedExpandData = onCustomizeExpandedNavigatonProperty(targetEntityType,
                                                expandType, expandData);
                                        currentResultSetEntity.getExpandData().get(expandTypeFqn).add(customizedExpandData);
                                    }
                                }
                            }
                        }
                        boolean needsNextLink = query.isServersidePaging()
                                && targetEntitiesResult.size() == this.getSQLQueryBuilder().getEntityPagingSize(targetEntityType);
        ...
        return OData2Utils.writeFeedWithExpand(getContext(), (UriInfo) uriInfo, targetEntitiesResult, contentType, count, nextLink);
    }
    ...
    public List<String> readIdsForExpand(final GetEntitySetUriInfo uriInfo) throws ODataException {
        ...
        try {
            ...
            try {
                ...
                PreparedStatement statement = null;
                try {
                    statement = createStatement(queryForIdsInExpand, connection);
                    resultSet = statement.executeQuery();
                    queryForIdsInExpand.setOffset(resultSet);
                    try {
                        while (queryForIdsInExpand.next(resultSet)) {//TODO remove the duplication here
                            //we select only the ids here (we assume that only one key property is in the ID)
    ...
    protected Map<String, Object> readResultSet(SQLQuery selectEntityQuery, final EdmStructuralType entityType,
            Collection<EdmProperty> properties, final ResultSet resultSet) throws EdmException, SQLException, ODataException {
    ...
    protected Object readProperty(EdmStructuralType entityType, EdmProperty property, SQLQuery selectEntityQuery, ResultSet resultSet)
