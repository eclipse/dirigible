...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = SqlFactory.getNative(new PostgresSqlDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", SqlFactory.getNative(new PostgresSqlDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
				.where("COMPANY = 'SAP'")
				.build();
	...
	public void updateWhereExpr() {
		String sql = SqlFactory.getNative(new PostgresSqlDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(SqlFactory.getNative(new PostgresSqlDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SAP'").build())
				.build();
