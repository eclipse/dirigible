...
public class PersistenceManager<T> {
        ...
        if (GenerationType.SEQUENCE.name()
          ...
          if (dialect.isSequenceSupported()) {
            ...
            throw new IllegalArgumentException(
                format("Generation Type: [{0}] not supported for database: [{1}] by using the dialect: [{2}].", columnModel.getGenerated(),
                    dialect.getDatabaseName(connection), dialect.getClass()
                                                                .getSimpleName()));
          }
  ...
  public List<T> query(Connection connection, Class<T> clazz, String sql, List<Object> values) {
    if (logger.isTraceEnabled()) {
      logger.trace("query -> connection: " + connection.hashCode() + ", class: " + clazz.getCanonicalName() + ", sql: " + sql + ", values: "
  ...
  public int execute(Connection connection, String sql, List<Object> values) {
    if (logger.isTraceEnabled()) {
      logger.trace(
          "execute -> connection: " + connection.hashCode() + ", sql: " + sql + ", values: " + Serializer.serializeListOfObjects(values));
    }
