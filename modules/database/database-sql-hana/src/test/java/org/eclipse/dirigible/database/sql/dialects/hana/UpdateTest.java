...
public class UpdateTest {
	...
	public void updateSimple() {
		String sql = SqlFactory.getNative(new HanaSqlDialect()).update().table("CUSTOMERS").set("FIRST_NAME", "'John'").build();
	...
	public void updateWhereSelect() {
		String sql = SqlFactory	.getNative(new HanaSqlDialect())
								...
								.set("FIRST_NAME", "'John'")
								.set("SALARY",
										SqlFactory.getNative(new HanaSqlDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
								.where("COMPANY = 'SAP'")
								.build();
