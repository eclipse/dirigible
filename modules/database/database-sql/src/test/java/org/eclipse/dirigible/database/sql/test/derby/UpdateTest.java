...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = SqlFactory.getNative(new DerbySqlDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", SqlFactory.getNative(new DerbySqlDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
				.where("COMPANY = 'SAP'")
				.build();
	...
	public void updateWhereExpr() {
		String sql = SqlFactory.getNative(new DerbySqlDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(SqlFactory.getNative(new DerbySqlDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SAP'").build())
				.build();
