...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = SqlFactory.getNative(new SnowflakeSqlDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", SqlFactory.getNative(new SnowflakeSqlDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
				.where("COMPANY = 'SNOWFLAKE'")
				.build();
			
			assertNotNull(sql);
			assertEquals("UPDATE CUSTOMERS SET FIRST_NAME = 'John', SALARY = SELECT MAX(SALARY) FROM BENEFITS WHERE (COMPANY = 'SNOWFLAKE')", sql);
	}
	...
	public void updateWhereExpr() {
		String sql = SqlFactory.getNative(new SnowflakeSqlDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(SqlFactory.getNative(new SnowflakeSqlDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SNOWFLAKE'").build())
				.build();
			
			assertNotNull(sql);
			assertEquals("UPDATE CUSTOMERS SET FIRST_NAME = 'John', LAST_NAME = 'Smith' WHERE (PRICE > ? OR AMOUNT < ? AND COMPANY = 'SNOWFLAKE')", sql);
	}
