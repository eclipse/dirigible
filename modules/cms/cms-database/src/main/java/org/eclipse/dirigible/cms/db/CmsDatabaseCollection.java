 ...
 */
public class CmsDatabaseCollection extends CmsDatabaseEntity implements ICollection {
	 ...
	 *            the path
	 */
	public CmsDatabaseCollection(CmsDatabaseRepository repository, RepositoryPath path) {
	...
	public void create() throws RepositoryWriteException {
		...
		if (parent == null) {
			throw new CmsDatabaseRepositoryException("Cannot create root collection.");
		}
	...
	public void delete() throws RepositoryWriteException {
		final CmsDatabaseFolder folder = getFolderSafe();
		try {
			folder.deleteTree();
	...
	public List<String> getCollectionsNames() throws RepositoryReadException {
		...
		final CmsDatabaseFolder folder = getFolderSafe();
		try {
			for (CmsDatabaseObject child : folder.getChildren()) {
				if (child instanceof CmsDatabaseFolder) {
	...
	public List<String> getResourcesNames() throws RepositoryReadException {
		...
		try {
			for (CmsDatabaseObject child : folder.getChildren()) {
				if (child instanceof CmsDatabaseFile) {
					result.add(child.getName());
				}
			}
		} catch (CmsDatabaseRepositoryException ex) {
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof CmsDatabaseCollection)) {
			return false;
		}
		final CmsDatabaseCollection other = (CmsDatabaseCollection) obj;
	 ...
	 */
	protected CmsDatabaseFolder getFolder() throws RepositoryReadException {
		final CmsDatabaseObject object = getLocalObject();
		if (object == null) {
			return null;
		}
		if (!(object instanceof CmsDatabaseFolder)) {
			return null;
		}
		return (CmsDatabaseFolder) object;
	}
	 ...
	 */
	protected CmsDatabaseFolder getFolderSafe() throws RepositoryNotFoundException {
		final CmsDatabaseFolder folder = getFolder();
		if (folder == null) {
