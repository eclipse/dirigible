...
public class CmsDatabaseResource extends CmsDatabaseEntity implements IResource {
	 ...
	 *            the path
	 */
	public CmsDatabaseResource(CmsDatabaseRepository repository, RepositoryPath path) {
		super(repository, path);
		try {
			CmsDatabaseFile localFile = getDocument();
			if (localFile != null) {
	...
	public void delete() throws RepositoryWriteException {
		final CmsDatabaseFile document = getDocumentSafe();
		try {
			document.delete();
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof CmsDatabaseResource)) {
			return false;
		}
		final CmsDatabaseResource other = (CmsDatabaseResource) obj;
	...
	/**
	 * Returns the {@link CmsDatabaseFile} object matching this {@link CmsDatabaseResource}. If
	 ...
	 */
	protected CmsDatabaseFile getDocument() throws RepositoryReadException {
		final CmsDatabaseObject object = getLocalObject();
		if (object == null) {
			return null;
		}
		if (!(object instanceof CmsDatabaseFile)) {
			return null;
		}
		return (CmsDatabaseFile) object;
	}
	 ...
	 */
	protected CmsDatabaseFile getDocumentSafe() throws RepositoryNotFoundException {
		final CmsDatabaseFile document = getDocument();
		if (document == null) {
