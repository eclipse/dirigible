...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = Squle.getNative(new SybaseSquleDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", Squle.getNative(new SybaseSquleDialect()).select().column("MAX(SALARY)").from("BENEFITS").toString())
				.where("COMPANY = 'SAP'")
				.toString();
	...
	public void updateWhereExpr() {
		String sql = Squle.getNative(new SybaseSquleDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(Squle.getNative(new SybaseSquleDialect()).expr().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SAP'").toString())
				.toString();
