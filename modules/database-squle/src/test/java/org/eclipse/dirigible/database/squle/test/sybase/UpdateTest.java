...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = Squle.getNative(new SybaseSquleDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", Squle.getNative(new SybaseSquleDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
				.where("COMPANY = 'SAP'")
				.build();
	...
	public void updateWhereExpr() {
		String sql = Squle.getNative(new SybaseSquleDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(Squle.getNative(new SybaseSquleDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SAP'").build())
				.build();
