...
public class SelectTest {
	...
	public void selectWhereExpr() {
		String sql = Squle.getNative(new H2SquleDialect())
			...
			.column("*")
			.from("CUSTOMERS")
			.where(Squle.getNative(new H2SquleDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").build())
			.build();
	...
	public void selectUnion() {
		String sql = Squle.getNative(new H2SquleDialect())
			...
			.column("COUNTRY")
			.from("CUSTOMERS")
			.union(Squle.getNative(new H2SquleDialect()).select().column("COUNTRY").from("SUPPLIERS").build())
			.build();
