...
public class UpdateTest {
	...
	public void updateWhereSelect() {
		String sql = Squle.getNative(new PostgresSquleDialect())
				...
				.table("CUSTOMERS")
				.set("FIRST_NAME", "'John'")
				.set("SALARY", Squle.getNative(new PostgresSquleDialect()).select().column("MAX(SALARY)").from("BENEFITS").build())
				.where("COMPANY = 'SAP'")
				.build();
	...
	public void updateWhereExpr() {
		String sql = Squle.getNative(new PostgresSquleDialect())
				...
				.set("FIRST_NAME", "'John'")
				.set("LAST_NAME", "'Smith'")
				.where(Squle.getNative(new PostgresSquleDialect()).expression().and("PRICE > ?").or("AMOUNT < ?").and("COMPANY = 'SAP'").build())
				.build();
