    ...
    .factory('Views', ['$resource', function ($resource) {
        ...
        let get = function () {
            if (cachedViews) return cachedViews;
                ...
                return $resource('/services/v4/js/${projectName}/gen/ui/core/services/views.js').query().$promise
                    .then(function (data) {
                        data = data.map(function (v) {
                            if (!v.id) {
                                console.error(`Views: view '${dollar}{v.label || 'undefined'}' does not have an id`);
                                return;
                            }
                            if (!v.label) {
                                console.error(`Views: view '${dollar}{v.id}' does not have a label`);
                                return;
                            }
                            if (!v.link) {
                                console.error(`Views: view '${dollar}{v.id}' does not have a link`);
                                return;
                            }
                ...
                pre: function (scope) {
                    ...
                    Views.get().then(function (views) {
                        ...
                        if (embeddedView) {
                            ...
                            throw Error(`embeddedView: view with id '${dollar}{scope.viewId}' not found`);
                        }
                    });
