...
problemsView.controller('ProblemsController', ['$http', '$timeout', 'messageHub', function ($http, $timeout, messageHub) {
    ...
    const fetchData = (args) => {
        ...
        $http.get('/services/v4/ops/problems/search', { params: { 'condition': this.filterBy, 'limit': limit } })
            .then((response) => {
    ...
    this.deleteSelected = function () {
        ...
        if (selectedIds.length > 0) {
            $http.post('/services/v4/ops/problems/delete/selected', selectedIds).then(() => {
                fetchData();
            });
        }
    }
