...
exports.create = function () {
	let commentsDao = require("ide-discussions/lib/comment_dao").create();
	let commentsSvc = rsdata.service(undefined, undefined, undefined, 'ide-discussions.svc.CommentsService').dao(commentsDao);
	commentsSvc.mappings().query().postQuery = function (entities, context) {
		let asDiscussionThread = (typeof context.queryParams.thread === 'string' && (context.queryParams.thread === 'true' || context.queryParams.thread.length === 0)) || ((typeof context.queryParams.thread === 'boolean') ? true : false) ? true : false;
