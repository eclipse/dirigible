...
let BoardsORM = exports.BoardsORM = {
...
let visit = function (boardId) {
	this.$log.info('Updating {}}[{}] entity visits', this.orm.table, boardId);
	let qb;
	try {
		let visitsField = this.orm.getProperty('visits');
		...
		let updatedRecordCount = this.execute(qb, params);
...
let setTags = function (id, tags, createOnDemand) {
	let tagRefsDAO = this.orm.getAssociation('tagRefs').targetDao();
	...
	let boardTags = tagRefsDAO.list(listSettings);
	if (boardTags) {
		for (let i = 0; i < boardTags.length; i++) {
			...
			let qb = require('db/v4/sql').getDialect()
				.delete()
			...
			let updatedRecordsCount = this.execute(qb, parameterBindings);
	...
	let tagsDAO = this.orm.getAssociation('tags').targetDao();
	for (let i = 0; i < tags.length; i++) {
		if (tags[i] !== null || tags[i] !== undefined) {
			let tagEntity = tagsDAO.list({
				'defaultLabel': tags[i],
				"namespace": TAGS_NAMESPACE
			})[0];
			let tagId = tagEntity && tagEntity[tagsDAO.orm.getPrimaryKey().name];
			...
			entity['boardId'] = id;
			entity['tagId'] = tagId;
			let boardTagId = tagRefsDAO.insert(entity);
...
exports.create = function () {
	let boardDAO = require('db/v4/dao').create(BoardsORM, 'BoardsDAO');
