...
public class ContentInitializerServlet extends HttpServlet {
	...
	private static final String CONTENT_FILE_NAME = "/content.zip"; //$NON-NLS-1$
	...
	public boolean initDefaultContent(HttpServletRequest request) throws ServletException {
		...
		try {
			...
			if (!resource.exists()) {
				...
				contentImporterServlet.importZipAndUpdate(this.getClass().getResourceAsStream(CONTENT_FILE_NAME), PATH_REGISTY_ROOT_TARGET, request);
