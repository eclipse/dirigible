...
public class DefinitionEnumerator {
	...
	public DefinitionEnumerator(final String repositoryPath, final ICollection collection, final String fileExtension) throws IOException {
	...
	private void enumerateJsDefinitions(final String repositoryPath, final ICollection collection, final String fileExtension) throws IOException {
				...
				if ((resource != null) && (resource.getName() != null)) {
					if (resource.getName().endsWith(fileExtension)) {
						...
						if (collecationPath.length() >= repositoryPath.length()) {
							final String fullPath = collecationPath.substring(repositoryPath.length()) + IRepository.SEPARATOR + resource.getName();
