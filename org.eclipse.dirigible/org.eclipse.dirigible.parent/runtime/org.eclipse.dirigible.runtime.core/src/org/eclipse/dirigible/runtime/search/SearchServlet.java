...
public class SearchServlet extends AbstractRegistryServlet {
	...
	private void enumeratePaths(final HttpServletRequest request, final HttpServletResponse response, final List<String> paths) throws IOException {
		final PrintWriter writer = response.getWriter();

		final String collectionPath = getContentLocationPrefix();

		final JsonArray rootArray = new JsonArray();

		for (final String entity : paths) {
			String entityName = entity;
			String deployPrefix = getContentDeployPrefix(request);
			if (entityName.startsWith(deployPrefix)) {
	...
	protected String getContentLocationPrefix() {
		final String collectionPath = "../" + IRepositoryPaths.REGISTRY; //$NON-NLS-1$
