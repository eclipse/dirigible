...
public abstract class AbstractScriptingServlet extends HttpServlet {
	...
	public static final String REGISTRY_SCRIPTING_DEPLOY_PATH = IRepositoryPaths.REGISTRY_DEPLOY_PATH + ICommonConstants.SEPARATOR
			+ ICommonConstants.ARTIFACT_TYPE.SCRIPTING_SERVICES;

	public static final String REGISTRY_INTEGRATION_DEPLOY_PATH = IRepositoryPaths.REGISTRY_DEPLOY_PATH + ICommonConstants.SEPARATOR
			+ ICommonConstants.ARTIFACT_TYPE.INTEGRATION_SERVICES;

	public static String getSandboxScripting(HttpServletRequest request) {
		return IRepositoryPaths.SANDBOX_DEPLOY_PATH + ICommonConstants.SEPARATOR + RepositoryFacade.getUser(request) + ICommonConstants.SEPARATOR
				+ ICommonConstants.ARTIFACT_TYPE.SCRIPTING_SERVICES;
	}

	public static final String REPOSITORY_ATTRIBUTE = RepositoryFacade.REPOSITORY; //"org.eclipse.dirigible.services.scripting.repository"; //$NON-NLS-1$
	...
	protected abstract void doExecution(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException;
	...
	protected String getScriptingRegistryPath(HttpServletRequest request) {
		if ((request != null) && (((request.getAttribute(SandboxFilter.SANDBOX_CONTEXT) != null)
				...
				|| ((request.getAttribute(SandboxFilter.DEBUG_CONTEXT) != null) && (Boolean) request.getAttribute(SandboxFilter.DEBUG_CONTEXT)))) {
