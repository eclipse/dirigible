...
public class RegistryMenuServlet extends AbstractRegistryServlet {
	...
	public void doGet(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			final IEntity entity = getEntity(IRepositoryPaths.DB_DIRIGIBLE_REGISTRY_CONF + IRepositoryPaths.REGISTRY + IRepositoryPaths.SEPARATOR
					+ IRepositoryPaths.UI + IRepositoryPaths.SEPARATOR + MENU_JSON, request);
			byte[] data;
			if (entity != null) {
			...
			sendData(out, "[{\"name\": \"Error\",\"link\":\"http://bugs.dirigible.io\"}]".getBytes());
		} finally {
			out.flush();
			out.close();
		}
