...
public class JavaServlet extends AbstractScriptingServlet {
	...
	public void init() throws ServletException {
		...
		if (libDirectory.exists() && (libDirectory.getParentFile() != null) && libDirectory.getParentFile().exists()) {
	...
	protected void doExecution(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (result != null) {
				...
				response.sendError(HttpServletResponse.SC_METHOD_NOT_ALLOWED, result.toString());
			}
	...
	public JavaExecutor createExecutor(HttpServletRequest request) throws IOException {
		JavaExecutor executor = new JavaExecutor(getRepository(request), getClasspath(), getScriptingRegistryPath(request),
