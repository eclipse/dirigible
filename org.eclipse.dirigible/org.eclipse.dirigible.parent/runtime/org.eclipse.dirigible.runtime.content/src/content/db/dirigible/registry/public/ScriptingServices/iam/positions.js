...
var positionsDao = require('iam/dao/positionsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = positionsDao.get(id);
			if (entity !== null) {
			...
			var positionsCount = positionsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', positionsCount);
		} else if (metadata !== null) {
			var positionsMetadata = positionsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(positionsMetadata, null, 2));
		} else {
			var positions = positionsDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(positions, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = positionsDao.create(entity);
