...
#foreach ($tableColumn in $tableColumns)#if($velocityCount > 1),#end${tableColumn.name}#end) VALUES (##
...
// Return all entities
exports.list = function(limit, offset, sort, desc, ${dependentColumn.toLowerCase()}) {
	if (${dependentColumn.toLowerCase()} === null) {
		return [];
	}
    var result = [];
    var connection = datasource.getConnection();
    try {
        ...
        var statement = connection.prepareStatement(sql);
        statement.setInt(1, ${dependentColumn.toLowerCase()});
        var resultSet = statement.executeQuery();
...
exports.count = function(${dependentColumn.toLowerCase()}) {
	if (${dependentColumn.toLowerCase()} === null) {
		return 0;
	}
    var count = 0;
