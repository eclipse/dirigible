...
var users_orgs_listDao = require('iam/dao/users_orgs_listDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var users_orgs_listCount = users_orgs_listDao.count(user.getName());
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', users_orgs_listCount);
		} else if (metadata !== null) {
			var users_orgs_listMetadata = users_orgs_listDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(users_orgs_listMetadata, null, 2));
		} else {
			var users_orgs_list = users_orgs_listDao.list(limit, offset, sort, desc, user.getName());
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(users_orgs_list, null, 2));
		}
	}
}
