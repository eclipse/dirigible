...
var usersDao = require('iam/dao/usersDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = usersDao.get(id);
			if (entity !== null) {
				sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
			} else {
				sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'user_id\'=' + id);
			}
		} else if (count !== null) {
			var usersCount = usersDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', usersCount);
		} else if (metadata !== null) {
			var usersMetadata = usersDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(usersMetadata, null, 2));
		} else {
			var users = usersDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(users, null, 2));
		}
	}
}
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		...
		sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'user_id\' parameter is missing!');
	}
...
function getIdParameter(httpRequest, xss) {
	...
	id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('user_id'));
	return id;
}
