...
var organizationsDao = require('iam/dao/organizationsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = organizationsDao.get(id);
			if (entity !== null) {
			...
			var organizationsCount = organizationsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', organizationsCount);
		} else if (metadata !== null) {
			var organizationsMetadata = organizationsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(organizationsMetadata, null, 2));
		} else {
			var organizations = organizationsDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(organizations, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = organizationsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		if (organizationsDao.get(id) !== null) {
			organizationsDao.update(entity);
