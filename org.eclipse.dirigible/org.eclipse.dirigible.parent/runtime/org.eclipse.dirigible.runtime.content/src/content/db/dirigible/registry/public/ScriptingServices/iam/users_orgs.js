...
var users_orgsDao = require('iam/dao/users_orgsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = users_orgsDao.get(id);
			if (entity !== null) {
			...
			var users_orgsCount = users_orgsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', users_orgsCount);
		} else if (metadata !== null) {
			var users_orgsMetadata = users_orgsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(users_orgsMetadata, null, 2));
		} else {
			var users_orgs = users_orgsDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(users_orgs, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = users_orgsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		if (users_orgsDao.get(id) !== null) {
			users_orgsDao.update(entity);
