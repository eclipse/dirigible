...
var assignDao = require('iam/dao/assignDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			var entity = assignDao.get(id);
			if (entity !== null) {
				sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
			} else {
				sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'assign_id\'=' + id);
			}
		} else if (count !== null) {
			var assignCount = assignDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', assignCount);
		} else if (metadata !== null) {
			var assignMetadata = assignDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(assignMetadata, null, 2));
		} else {
			var assign = assignDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(assign, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = assignDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	var entity = getRequestBody(httpRequest);
	var id = getIdParameter(httpRequest, xss);
	id = id !== null ? id : entity.assign_id;
	if (id !== null) {
		if (assignDao.get(id) !== null) {
			assignDao.update(entity);
			sendResponse(httpResponse, httpResponse.NO_CONTENT);
		} else {
			sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'assign_id\'=' + id);
		}
	} else {
		sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'assign_id\' parameter is missing!');
	}
...
function getIdParameter(httpRequest, xss) {
	...
	id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('assign_id'));
	return id;
}
