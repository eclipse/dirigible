...
var organizations_listDao = require('iam/dao/organizations_listDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var organizations_listCount = organizations_listDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', organizations_listCount);
		} else if (metadata !== null) {
			var organizations_listMetadata = organizations_listDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(organizations_listMetadata, null, 2));
		} else {
			var organizations_list = organizations_listDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(organizations_list, null, 2));
		}
	}
}
