...
var userDao = require('profile/dao/userDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			...
			var userCount = userDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', userCount);
		} else if (metadata !== null) {
			var userMetadata = userDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(userMetadata, null, 2));
		} else {
			var user = userDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(user, null, 2));
		}
	}
}
