...
public class ListenerLogCleanupTask implements IRunnableTask {

	private static final Logger logger = Logger.getLogger(ListenerLogCleanupTask.class);
	...
	public void start() {
		logger.debug("entering: " + this.getClass().getCanonicalName() + " -> " //$NON-NLS-1$ //$NON-NLS-2$
				+ "start()"); //$NON-NLS-1$
		try {
			ListenerLogRecordDAO.cleanupOlderRecords();
		} catch (Exception e) {
