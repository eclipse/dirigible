		...
		browser.addProgressListener(new ProgressListener() {
			...
			public void completed(final ProgressEvent event) {
				...
				if (javaScriptEditor && (DebugModelFacade.getDebugModel(CommonIDEParameters.getUserName()) != null)) {
					final DebugSessionModel session = DebugModelFacade.getDebugModel(CommonIDEParameters.getUserName()).getActiveSession();
					...
					if ((session != null) && (session.getCurrentLineBreak() != null)) {
						...
						final String path = CommonIDEUtils.formatToRuntimePath(ICommonConstants.ARTIFACT_TYPE.SCRIPTING_SERVICES, filePath);
						final int[] breakpoints = DebugModelFacade.getDebugModel(CommonIDEParameters.getUserName()).getBreakpointsMetadata()
								.getBreakpoints(path);
