...
public class ResultSetStringWriter implements ResultSetWriter<String>{
	...
	public String writeTable(ResultSet resultSet) throws SQLException {
		...
		while (resultSet.next()) {
			...
			resultSetMetaData = resultSet.getMetaData();//needs an update on each iteration for nosql dbs as it can change with each document
			...
			for (String headerForRow : headersForRow) {
				...
				colDescr.setDisplaySize(resultSetMetaData.getColumnDisplaySize(colindex));
