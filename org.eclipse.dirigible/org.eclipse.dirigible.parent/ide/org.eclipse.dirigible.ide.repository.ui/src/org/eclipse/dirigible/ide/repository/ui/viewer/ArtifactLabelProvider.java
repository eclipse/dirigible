...
public class ArtifactLabelProvider extends AbstractArtifactLabelProvider {
	...
	protected Image getEntityImage(IEntity entity) {
		if (entity instanceof IResource) {
			final String resourceName = ((IResource) entity).getName().toLowerCase();
	...
	protected Image getCollectionImageByName(String name) {
		...
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.EXTENSION_DEFINITIONS)) {
			return createImage(TYPE_COLLECTION_EXT_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.INTEGRATION_SERVICES)) {
			return createImage(TYPE_COLLECTION_IS_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.SCRIPTING_SERVICES)) {
			return createImage(TYPE_COLLECTION_SS_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.SECURITY_CONSTRAINTS)) {
			return createImage(TYPE_COLLECTION_SEC_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.TEST_CASES)) {
			return createImage(TYPE_COLLECTION_TEST_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.WEB_CONTENT)) {
			return createImage(TYPE_COLLECTION_WEB_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.WIKI_CONTENT)) {
			return createImage(TYPE_COLLECTION_WIKI_ICON_URL);
		} else if (name.equals(ICommonConstants.ARTIFACT_TYPE.MOBILE_APPLICATIONS)) {
			return createImage(TYPE_COLLECTION_MOBILE_ICON_URL);
		}
		return createImage(TYPE_COLLECTION_ICON_URL);
	}
