 ...
 * performance of autobuilds, etc.) are generally deferred until the outermost
 * operation has successfully completed. The platform may still decide to
 * broadcast periodic resource change notifications during the scope of the
 * operation if the operation runs for a long time or another thread modifies
 ...
 * rule is provided, the operation will obtain a scheduling rule that locks the
 * entire workspace for the duration of the operation.
 * </p>
 * <p>
 ...
 * 
 * @see ISchedulingRule
 * @see org.eclipse.core.resources.IWorkspace#run(IWorkspaceRunnable,
 *      IProgressMonitor)
 * */
public abstract class WorkspaceModifyOperation implements
	...
	public void threadChange(Thread thread) {
		// we must make sure we aren't transferring control away from a thread
		// that
		// already owns a scheduling rule because this is deadlock prone (bug
		// 105491)
		if (rule == null) {
			return;
		}
		...
		throw new IllegalStateException(
				CANNOT_FORK_A_THREAD_FROM_A_THREAD_OWNING_A_RULE); //$NON-NLS-1$
	}
