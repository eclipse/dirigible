...
/**
 * This popup dialog is used for configure presentation properties.
 */
public class ConfigurationDialog extends PopupDialog {
	...
	protected void adjustBounds() {
		...
		getShell().setBounds((bounds.x + (bounds.width / 2)) - (newWidth / 2), (bounds.y + (bounds.height / 2)) - (newHeight / 2), newWidth,
				newHeight);
	}
	...
	private void loadActionSettings(final Composite container) {
		...
		if (manager != null) {
			...
			if (manager instanceof ViewToolBarManager) {
				...
				for (ItemData element : itemsData) {
