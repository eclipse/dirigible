...
public class FancyWindowComposer implements IWindowComposer {
	...
	public Composite createWindowContents(final Shell shell, final IWorkbenchWindowConfigurer configurer) {
		...
		ElementBuilder dummyBuilder = new DummyBuilder(null, ILayoutSetConstants.SET_ID_MENUBAR);
	...
	private void attachPageBg(final FormData fdPageBg, final Composite menuBarComp) {
		if (configurer.getShowMenuBar()) {
	...
	private void createPerspectiveBar(final Composite header) {
		...
		ElementBuilder perspBuilder = new PerspectiveSwitcherBuilder(perspBar, ILayoutSetConstants.SET_ID_PERSP);
		perspBuilder.build();
	}
	...
	private void styleButtons(final Control[] buttons) {
		for (Control button : buttons) {
			if (button instanceof Button) {
				button.setData(RWT.CUSTOM_VARIANT, "clearButton"); //$NON-NLS-1$
			}
		}
	}
