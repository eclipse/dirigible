...
public class CoolBarManager extends CoolBarManager2 {
	...
	public void update(final boolean force) {
		if ((isDirty() || force) && getControl2() != null) {
			...
			final List<IContributionItem> visibleItems = new ArrayList<IContributionItem>(
					items.length);
			for (int i = 0; i < items.length; i++) {
				...
				if (item.isVisible()) {
					if (item instanceof IToolBarContributionItem) {
						...
						IToolBarManager toolBarManager = toolbarItem
								.getToolBarManager();
						IContributionItem[] toolbarItems = toolBarManager
								.getItems();
			...
			final ArrayList<ToolItem> toolItemsToRemove = new ArrayList<ToolItem>(
					toolItems.length);
			...
			final Iterator<IContributionItem> visibleItemItr = visibleItems
					.iterator();
			while (visibleItemItr.hasNext()) {
				...
				StylingSelectionAdapter listener = new StylingSelectionAdapter(
						HEADER_TOOLBAR_VARIANT);
	...
	private int getChildrenSize(final ToolBar toolbar) {
		...
		FormData spacing = dummyBuilder
				.getPosition(LayoutSetConstants.COOLBAR_SPACING);
		if (spacing != null) {
	...
	private FormData getOverflowButtonLayoutData() {
		...
		FormData fdOverFlowButton = dummyBuilder
				.getPosition(LayoutSetConstants.COOLBAR_BUTTON_POS);
		if (image != null) {
	...
	private void createOverflowLayer() {
		ElementBuilder layerBuilder = new CoolbarLayerBuilder(
				overflowParent.getParent(), LayoutSetConstants.SET_ID_OVERFLOW);
	...
	private void styleToolItems() {
		if (toolbar != null) {
			...
			for (int i = 0; i < items.length; i++) {
				ToolItem toolItem = items[i];
				final IContributionItem item = (IContributionItem) toolItem
						.getData();
				if (toolItem.getText() == "") { //$NON-NLS-1$
					modifyModeForceText(item);
				}
				toolItem.setData(WidgetUtil.CUSTOM_VARIANT,
						HEADER_TOOLBAR_VARIANT);
			}
		}
	}
