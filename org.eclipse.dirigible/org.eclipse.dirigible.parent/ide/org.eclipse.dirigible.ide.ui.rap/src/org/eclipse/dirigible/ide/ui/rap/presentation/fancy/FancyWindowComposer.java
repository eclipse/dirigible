...
import org.eclipse.dirigible.ide.ui.rap.builders.FooterBuilder;
import org.eclipse.dirigible.ide.ui.rap.builders.PerspectiveSwitcherBuilder;
import org.eclipse.dirigible.ide.ui.rap.builders.fancy.HeaderBuilder;
...
public class FancyWindowComposer implements IWindowComposer {
	...
	public Composite createWindowContents(final Shell shell,
		...
		ElementBuilder dummyBuilder = new DummyBuilder(null,
				LayoutSetConstants.SET_ID_MENUBAR);
		Image bg = dummyBuilder.getImage(LayoutSetConstants.MENUBAR_BG);
	...
	private void createStatusLine(final Composite parent) {
		...
		statusLineComp.addControlListener(new ControlAdapter() {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 4139822819115230387L;
	...
	private void createFooter(final Composite pageBg) {
		...
		if (configurer.getShowStatusLine()) {
			...
			ElementBuilder footerBuilder = new FooterBuilder(footer,
					LayoutSetConstants.SET_ID_FOOTER);
			footerBuilder.build();
			...
			if (configurer.getShowProgressIndicator()) {
				...
				progressRegion.createContents(statusLineParent,
	...
	private void createHeader() {
		...
		ElementBuilder headerBuilder = new HeaderBuilder(headerArea,
				LayoutSetConstants.SET_ID_HEADER);
		headerBuilder.build();
	...
	private void createPerspectiveBar(final Composite header) {
		...
		// XXX set to 70 instead to 0
		fdPerspBar.left = new FormAttachment(0, 350);
		// fdPerspBar.right = new FormAttachment( 100, 50 );
		fdPerspBar.top = new FormAttachment(0, 0);
		fdPerspBar.width = 1000;
		...
		ElementBuilder perspBuilder = new PerspectiveSwitcherBuilder(perspBar,
				LayoutSetConstants.SET_ID_PERSP);
		perspBuilder.build();
	}
