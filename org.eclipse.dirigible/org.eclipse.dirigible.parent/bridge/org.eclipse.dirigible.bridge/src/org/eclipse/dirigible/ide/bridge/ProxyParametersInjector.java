...
public class ProxyParametersInjector implements IInjector {
	...
	public void injectOnRequest(ServletConfig servletConfig, HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (parameterHTTP_PROXY_HOST != null) {
			req.setAttribute(HTTP_PROXY_HOST, parameterHTTP_PROXY_HOST);
		...
		if (parameterHTTP_PROXY_PORT != null) {
			req.setAttribute(HTTP_PROXY_PORT, parameterHTTP_PROXY_PORT);
		...
		if (parameterHTTPS_PROXY_HOST != null) {
			req.setAttribute(HTTPS_PROXY_HOST, parameterHTTPS_PROXY_HOST);
		...
		if (parameterHTTPS_PROXY_PORT != null) {
			req.setAttribute(HTTPS_PROXY_PORT, parameterHTTPS_PROXY_PORT);
		...
		if (parameterHTTP_NON_PROXY_HOSTS != null) {
			req.setAttribute(HTTP_NON_PROXY_HOSTS, parameterHTTP_NON_PROXY_HOSTS);
