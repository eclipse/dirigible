...
public interface IDialectSpecifier {
	 ...
	 * Used in Dirigible system database scripts for database agnostic SQL statements that are tailored at runtime to
	 * specific dialects.
	 */
	 ...
	 * <CODE>
	 * return sql.replace(IDialectSpecifier.DIALECT_TIMESTAMP, "TIMESTAMP").replace(IDialectSpecifier.DIALECT_BLOB, "BLOB").replace(... </CODE>
	 * and so on
	 *
	 * @param sql
	 *            an input SQL statement string potentially containing variables to be replaced
	 * @return a processed SQL statement string with variables replaced by dialect-specific values
	 */
	String specify(String sql);

	/**
	 * The method returns the most suitable dialect-specific SQL type that corresponds to the provided
	 * <CODE>commonType</CODE> parameter.
	 * Different mechanisms in Dirigible make use of this method to translate the common data types they use to dialect
	 * specific ones.
	 *
	 * @param commonType
	 *            one of the types returned by {@link DBSupportedTypesMap#getSupportedTypes()} (VARCHAR, CHAR, INTEGER,
	 *            BIGINT, SMALLINT, REAL, DOUBLE, DATE, TIME, TIMESTAMP, BLOB)
	 * @return The database-specific SQL type that corresponds to the commonType parameter, if any, or the provided
	 *         commonType if it's the same.
	 */
	String getSpecificType(String commonType);

	/**
	 * Provides a dialect specific <CODE>LIMIT</CODE> and/or <CODE>OFFSET</CODE> syntactic construction that is appended
	 * to an SQL query.
	 * Normally, this is alternative to {@link #createTopAndStart(int, int)}.
	 *
	 * @param limit
	 *            a limit parameter to insert in the correct place in the <CODE>LIMIT</CODE> syntactic construct part of
	 *            an SQL statement
	 * @param offset
	 *            an offset parameter to insert in the correct place in the <CODE>OFFSET</CODE> syntactic construct part
	 *            of an SQL statement
	...
	/**
	 * Provides a dialect specific <CODE>TOP</CODE> and/or <CODE>START</CODE> syntactic construction that is inserted in
	 * an SQL query.
	 * Normally, this is alternative to createLimitAndOffset(int, int).
	 *
	 * @param limit
	 *            a limit parameter to insert in the correct place in the <CODE>TOP</CODE> syntactic construct part of
	 *            an SQL statement
	 * @param offset
	 *            an offset parameter to insert in the correct place in the <CODE>START</CODE> syntactic construct part
	 *            of an SQL statement
	 ...
	 *
	 * @param productName
	 *            the string provided by the database JDBC driver's {@link DatabaseMetaData#getDatabaseProductName()}
	 *            method
	 * @return true if this dialect is applicable for databases with the supplied <CODE>productName</CODE> or flase
	 *         otherwise.
	 */
	 ...
	 * <CODE>SELECT * FROM &lt;schema-name&gt;.&lt;table-name&gt;</CODE>.
